...
class CartViewController:  UIViewController, UITableViewDelegate, UITableViewDataSource, CartViewModelAction  {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let productPrice = cartViewModel?.cartDetails?.data.checkout.id.count{
            let priceString = String(productPrice)
            cell.productInCartDescription.text = "\(priceString)"
            cell.productInCartDescription.font = UIFont.boldSystemFont(ofSize: 15)
        }
